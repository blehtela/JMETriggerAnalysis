...
#  ROOT.TH1.AddDirectory(False)
  ...
  recosList = ['test_noCustom', 'test_wrongJECs', 'test_correctJECs']           #needed to define input files later. NOTE: instead of this we could also directly use a file-list (in my case only one file, could give as input argument)
  ...
  for _tmpJetThresh in ['60','140', '320', '500']:
    ...
    for _tmpRef in [
      ...
      for _tmpType in [
        ...
        #here we assume the paths to have HLTPFJet structure, we try the different thresholds (so won't work for my PFPuppiJet path)
        #insetad of using the _tmpJetThresh, we could also just directly define the paths.
        # "styling dict"
        try:

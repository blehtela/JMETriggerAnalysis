...
for bpix_cat in bpix_categories:
    ...
    for jet_cat in jet_categories:
        file_name = f"sub_jecs_total_{bpix_cat}_{jet_cat}.htc"
        with open(filename, "w") as job_script: #different name for my testing phase
            ...
            job_script.write(f"output = htc_out_2024/{bpix_cat}_{jet_cat}.out\n")
            job_script.write(f"error = htc_out_2024/{bpix_cat}_{jet_cat}.err\n")
            job_script.write(f"log = htc_out_2024/{bpix_cat}_{jet_cat}.log\n")
            ...
            print("Submitting: condor_submit %s",file_name)
            os.system(f'condor_submit {file_name}')
